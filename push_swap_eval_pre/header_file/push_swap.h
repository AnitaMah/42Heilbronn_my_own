...
/*
 * t_flags - Represents the program's configuration state.
 * This structure holds the settings determined by command-line arguments.
 * It manages the chosen sorting strategy, enables/disables the 
 * performance benchmarking mode, and controls the output format by 
 * toggling the count-only display. It is passed throughout the 
 * program to ensure consistency in behavior.
 */
typedef struct s_flags
{
    int             strategy;
...
/*
 * print_count_only - Outputs only the total operation count to stdout.
 * When the 'count_only' flag is active, this function calculates 
 * the final tally of operations and writes it to the standard output. 
 * This is primarily for testing scripts that only care about 
 * performance metrics rather than the operation logs.
 */
